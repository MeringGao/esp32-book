...
解码：
你可以修改"访问位"，数据块和扇区尾部块的表格将自动更新。

编码：
点击表格每行的"编辑"按钮来选择你偏好的访问条件。这将更新访问位。

> [!Caution]
> 向访问条件位写入错误的值可能导致扇区无法访问。
...
<table border="1" id="db-table">
  ...
  <tbody>
    <tr data-row="1" id="db-row0">
      ...
      <td>key A|B</td>
      <td>默认配置</td>
      <td><button class="edit-button" onclick="toggleNestedTable(this, 'nested-table',10)">编辑</button></td>
    </tr>
...
<table border="1" id="st-table">
  ...
  <tbody>
    <tr data-row="1" id="st-row0"> 
        ...
        <td><button class="edit-button" onclick="toggleNestedTable(this, 'nested-st', 11)">编辑</button></td>
    </tr>
  </tbody>
</table>
...
- 此 UI 的灵感来自这个计算器：[Mifarecalc](https://gitlab.com/limentas/mifare-calc)
